...
class BookAdapter(_context: Context, _books: BookList, _vocl: View.OnClickListener) : RecyclerView.Adapter<BookAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = inflater.inflate(R.layout.book_list, null)
        return ViewHolder(view)
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val titleLabel = holder.view.findViewById<TextView>(R.id.bookTitleTextView)
        titleLabel.setText(books.get(position).title)
        val authorLabel = holder.view.findViewById<TextView>(R.id.bookAuthorTextView)
